...
void MultiplayerGameState::HandlePacket(uint8_t packet_type, sf::Packet& packet)
{
	...
	case Server::PacketType::kSpawnSelf:
	{
		...
		Tank* aircraft = m_world.AddAircraft(aircraft_identifier, m_players[aircraft_identifier]->GetDetails().m_colour);
	...
	case Server::PacketType::kAcceptCoopPartner:
	{
		...
		m_world.AddAircraft(aircraft_identifier, m_players[aircraft_identifier]->GetDetails().m_colour);
